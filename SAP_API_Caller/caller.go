...
import (
	...
	"io/ioutil"
	"net/http"
	sap_api_output_formatter "sap-api-integrations-contact-reads-rmq-kube-c4/SAP_API_Output_Formatter"
	"strings"
	"sync"
...
func (c *SAPAPICaller) ContactCollection(contactID string) {
	contactCollectionData, err := c.callContactSrvAPIRequirementContactCollection("ContactCollectionData", contactID)
	if err != nil {
		c.log.Error(err)
		return
	}
	err = c.outputter.Send(c.outputQueues[0], map[string]interface{}{"message": contactCollectionData, "function": "ContactCollectionData"})
	if err != nil {
		c.log.Error(err)
		return
	}
	...
	err = c.outputter.Send(c.outputQueues[0], map[string]interface{}{"message": contactIsContactPersonForData, "function": "ContactIsContactPersonForData"})
	if err != nil {
		c.log.Error(err)
		return
	}
	...
	err = c.outputter.Send(c.outputQueues[0], map[string]interface{}{"message": corporateAccountData, "function": "CorporateAccountData"})
	if err != nil {
		c.log.Error(err)
		return
	}
...
func (c *SAPAPICaller) IndividualCustomerCollection(customerID string) {
	individualCustomerCollectionData, err := c.callContactSrvAPIRequirementIndividualCustomerCollection("IndividualCustomerCollectionData", customerID)
	if err != nil {
		c.log.Error(err)
		return
	}
	err = c.outputter.Send(c.outputQueues[0], map[string]interface{}{"message": individualCustomerCollectionData, "function": "IndividualCustomerCollectionData"})
	if err != nil {
		c.log.Error(err)
		return
	}
	...
	err = c.outputter.Send(c.outputQueues[0], map[string]interface{}{"message": individualCustomerAddressData, "function": "IndividualCustomerAddressData"})
	if err != nil {
		c.log.Error(err)
		return
	}
...
func (c *SAPAPICaller) ContactIsContactPersonFor(accountFormattedName string) {
	contactIsContactPersonForData, err := c.callContactSrvAPIRequirementContactIsContactPersonFor("ContactIsContactPersonForCollectionData", accountFormattedName)
	if err != nil {
		c.log.Error(err)
		return
	}
...
func (c *SAPAPICaller) CorporateAccount(accountID string) {
	data, err := c.callContactSrvAPIRequirementCorporateAccount("CorporateAccountCollectionData", accountID)
	if err != nil {
		c.log.Error(err)
		return
	}
	err = c.outputter.Send(c.outputQueues[0], map[string]interface{}{"message": data, "function": "CorporateAccountData"})
	if err != nil {
		c.log.Error(err)
		return
	}
